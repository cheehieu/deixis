 ...
 * Button to read file and print coordinates
 ***************************************************************/
...
void wiiCallback(const joy::Joy::ConstPtr& joy)
{
    ...
    if(joy->buttons[3]==1)     //pressed 'B' to display number of recorded points
    {
        ...
        if (secs>0.05)
        {
            std::cout << "You recorded " << count << " marker points.\n";
            wiiRumble(-1,count,.03,0,0,.2);    //rumble count times
        }
    }
