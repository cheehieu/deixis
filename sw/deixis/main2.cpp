...
sensor_msgs::JointState*       g_joint_array     = NULL;
...
void jointStateCallback(const sensor_msgs::JointStateConstPtr &j);
...
int main(int argc, char** argv)
{
  ...
  ros::Subscriber joint_subscriber = nh.subscribe("joint_states", 0,jointStateCallback);
  ros::Publisher  joint_publisher  = nh.advertise<sensor_msgs::JointState>("target_joints", 1000);
  //sensor_msgs::JointState joint_array;
  //g_joint_array = &joint_array;
  
  g_joint_array = new sensor_msgs::JointState();
...
bool publishJointArray()
{
  ...
  {
    for (unsigned int i = 0 ; i < g_joint_array->name.size() ; i++)
    {
		int id;
		std::string name = g_joint_array->name[i];
		if(name == "bandit_head_tilt_joint") 						id = 0;
		else if(name == "bandit_head_pan_joint") 					id = 1;
		else if(name == "left_torso_shoulder_mounting_joint") 		id = 2;
		else if(name == "left_shoulder_mounting_shoulder_joint")	id = 3;
		else if(name == "left_shoulder_bicep_joint") 				id = 4;
		else if(name == "left_bicep_forearm_joint") 				id = 5;
		else if(name == "left_forearm_wrist_joint") 				id = 6;
		else if(name == "left_wrist_hand_joint") 					id = 7;
		else if(name == "left_hand_thumb_joint") 					id = 8;
		else if(name == "right_torso_shoulder_mounting_joint") 		id = 9;
		else if(name == "right_shoulder_mounting_shoulder_joint") 	id = 10;
		else if(name == "right_shoulder_bicep_joint") 				id = 11;
		else if(name == "right_bicep_forearm_joint") 				id = 12;
		else if(name == "right_forearm_wrist_joint")				id = 13;
		else if(name == "right_wrist_hand_joint") 					id = 14;
		else if(name == "right_hand_thumb_joint") 					id = 15;
		else if(name == "bandit_head_left_brow_joint")	 			id = 16;
		else if(name == "bandit_head_right_brow_joint")				id = 17;
		else if(name == "mouth_top_joint") 							id = 18;
		else if(name == "mouth_bottom_joint") 						id = 19;
		else
		{
			std::cout << "Joint name " << name << " is out of bounds.\n";
			continue;
		}

		if ((id >= 0) && (id < getNJoints()))
			g_angle_desired[id] = g_joint_array->position[i];
    }
  }
...
bool clearJointArray()
{
  ...
  g_joint_array->name.clear();
  g_joint_array->position.clear();
  g_joint_array->velocity.clear();
  g_joint_array->effort.clear();
  return true;
} // clearJointArray()
...
bool setInitPose()
{
  ...
  std::string joint_str_names[] =
  {
	  "bandit_head_tilt_joint", 
	  "bandit_head_pan_joint", 
	  "left_torso_shoulder_mounting_joint", 
	  "left_shoulder_mounting_shoulder_joint", 
	  "left_shoulder_bicep_joint", 
	  "left_bicep_forearm_joint", 
	  "left_forearm_wrist_joint", 
	  "left_wrist_hand_joint", 
	  "left_hand_thumb_joint", 
	  "right_torso_shoulder_mounting_joint", 
	  "right_shoulder_mounting_shoulder_joint", 
	  "right_shoulder_bicep_joint", 
	  "right_bicep_forearm_joint", 
	  "right_forearm_wrist_joint", 
	  "right_wrist_hand_joint", 
	  "right_hand_thumb_joint", 
	  "bandit_head_left_brow_joint", 
	  "bandit_head_right_brow_joint", 
	  "mouth_top_joint", 
	  "mouth_bottom_joint"
  };// joint_str_names[]

  // set the name and angle of each joint
  for (int i = 0; i < getNJoints(); ++i)
  {
	...
	g_joint_array->name.push_back(joint_str_names[i]);
	g_joint_array->position.push_back(joint_angles[i]);
	g_joint_array->velocity.push_back(0);
	g_joint_array->effort.push_back(0);
  }
...
bool setHeadPose(double head_pitch, double head_pan)
{
  ...
  // add head pitch angle to joint array
  g_joint_array->name.push_back("bandit_head_tilt_joint");
  g_joint_array->position.push_back(head_pitch);
  g_joint_array->velocity.push_back(0);
  g_joint_array->effort.push_back(0);

  // add head pan to joint array
  g_joint_array->name.push_back("bandit_head_pan_joint");
  g_joint_array->position.push_back(head_pan);
  g_joint_array->velocity.push_back(0);
  g_joint_array->effort.push_back(0);
                ...
                int    which_arm)
{
  ...
  // add shoulder FB to joint array
  if (which_arm == ARM_LEFT) 		g_joint_array->name.push_back("left_torso_shoulder_mounting_joint");
  else if (which_arm == ARM_RIGHT) 	g_joint_array->name.push_back("right_torso_shoulder_mounting_joint");
  g_joint_array->position.push_back(shoulder_fb);
  g_joint_array->velocity.push_back(0);
  g_joint_array->effort.push_back(0);

  // add shoulder IO to joint array
  if (which_arm == ARM_LEFT) 		g_joint_array->name.push_back("left_shoulder_mounting_shoulder_joint");
  else if (which_arm == ARM_RIGHT)  g_joint_array->name.push_back("right_shoulder_mounting_shoulder_joint");
  g_joint_array->position.push_back(shoulder_io);
  g_joint_array->velocity.push_back(0);
  g_joint_array->effort.push_back(0);

  // add elbow twist to joint array
  if (which_arm == ARM_LEFT) 		g_joint_array->name.push_back("left_shoulder_bicep_joint");
  else if (which_arm == ARM_RIGHT)  g_joint_array->name.push_back("right_shoulder_bicep_joint");
  g_joint_array->position.push_back(elbow_twist);
  g_joint_array->velocity.push_back(0);
  g_joint_array->effort.push_back(0);

  // add elbow to joint array
  if (which_arm == ARM_LEFT) 		g_joint_array->name.push_back("left_bicep_forearm_joint");
  else if (which_arm == ARM_RIGHT)	g_joint_array->name.push_back("right_bicep_forearm_joint");
  g_joint_array->position.push_back(elbow);
  g_joint_array->velocity.push_back(0);
  g_joint_array->effort.push_back(0);

  // add wrist twist to joint array
  if (which_arm == ARM_LEFT) 		g_joint_array->name.push_back("left_forearm_wrist_joint");
  else if (which_arm == ARM_RIGHT)	g_joint_array->name.push_back("right_forearm_wrist_joint");
  g_joint_array->position.push_back(wrist_twist);
  g_joint_array->velocity.push_back(0);
  g_joint_array->effort.push_back(0);

  // add wrist tilt to joint array
  if (which_arm == ARM_LEFT) 		g_joint_array->name.push_back("left_wrist_hand_joint");
  else if (which_arm == ARM_RIGHT)	g_joint_array->name.push_back("right_wrist_hand_joint");
  g_joint_array->position.push_back(wrist_tilt);
  g_joint_array->velocity.push_back(0);
  g_joint_array->effort.push_back(0);
  
  // add hand grab to joint array
  if (which_arm == ARM_LEFT) 		g_joint_array->name.push_back("left_hand_thumb_joint");
  else if (which_arm == ARM_RIGHT)	g_joint_array->name.push_back("right_hand_thumb_joint");
  g_joint_array->position.push_back(hand_grab);
  g_joint_array->velocity.push_back(0);
  g_joint_array->effort.push_back(0);
...
bool setEyebrowsPose(double eyebrows)
{
  ...
  // add left eyebrow to joint array
  g_joint_array->name.push_back("bandit_head_left_brow_joint");
  g_joint_array->position.push_back(eyebrows);
  g_joint_array->velocity.push_back(0);
  g_joint_array->effort.push_back(0);
  
  // add right eyebrow to joint array
  g_joint_array->name.push_back("bandit_head_right_brow_joint");
  g_joint_array->position.push_back(eyebrows);
  g_joint_array->velocity.push_back(0);
  g_joint_array->effort.push_back(0);
...
bool setMouthPose(double mouth_top, double mouth_bottom)
{
  ...
  // add mouth top to joint array
  g_joint_array->name.push_back("mouth_top_joint");
  g_joint_array->position.push_back(mouth_top);
  g_joint_array->velocity.push_back(0);
  g_joint_array->effort.push_back(0);
  
  // add mouth_bottom to joint array
  g_joint_array->name.push_back("mouth_bottom_joint");
  g_joint_array->position.push_back(mouth_bottom);
  g_joint_array->velocity.push_back(0);
  g_joint_array->effort.push_back(0);
...
bool generateDeicticEvents(DeicticEvents &deictic_events)
{
  ...
  std::cout << "Size of modalities: " << sizeof(MODALITIES) << std::endl;

  randomizeList(robot_poses);
  
  int multiple_n = 2;	//number of times to add each target, per modality
  for (int i=0 ; i < multiple_n ; i++)
  {
...
//read output.txt file and set targets, add deictic event
        ...
        while (getline(input,line))
            lineCount++;
        std::cout << "Found "<< lineCount << " recorded marker points in output.txt\n";
        input.close();
        ...
        for (int i=0;i<lineCount;i++)
        {
            ...
            deictic_event.target_pos.target_type = "SALIENT";
...
//		  for (int i = 0 ; i < ((120-(3*multiple_n*lineCount))/(2*3*multiple_n)) ; i++)
		  for (int i = 0; i < 5; i++)
          {
            // arm
            ...
            deictic_event.target_pos.target_type = "RANDOM";
            temp.push_back(deictic_event);
...
//          for (int i = 0; i < N_TARGETS_PER_MODALITY; ++i)
//		  for (int i = 0 ; i < ((120-(3*multiple_n*lineCount))/(3*multiple_n)) ; i++)
		  for (int i = 0; i < 10; ++i)
          {
                        ...
                        ostream       &out)
{
       ...
       ++de_iter)
  {
    out << de_iter->index      << ". "
        ...
        << " / " << de_iter->target_pos
        << " / " << de_iter->target_pos.target_type << endl;
  }
  printf("\n# of events: %d\n\n", int(deictic_events.size()));
  return true;
...
void jointStateCallback(const sensor_msgs::JointStateConstPtr &joints)
{
  ...
  for (unsigned int i = 0 ; i < joints->name.size() ; i++)
  {
    ...
    double angle_actual  = radiansToDegrees(joints->position[id]);
  ...
  if ((g_curr_event != NULL) && (g_mtx_curr_event != NULL))
  {
    ...
    printf("event_index        = %d of %d ", event_index, n_events - 1);
    std::cout << "(" << (*g_curr_event)->target_pos.target_type << ")\n";
    ...
    printf("=====================\n");
//    std::cout << g_joint_info->pos[1] << endl;
//    std::cout << g_joint_info->pos[9] << endl;
  }
} // jointStateCallback(const sensor_msgs::JointStateConstPtr &)
